%% 导入初始滤波参数 P、Q、R_const 2014.10.10
%   dataSource 
% 默认：isTrueX0=0

function [ P_ini,Q_const,R_ini,matchN_toplimit,NavFilterParameter ] = GetFilterParameter_SINSerror_FPcUKF( pg,ng,pa,na,NavFilterParameter )
%% 
dataSource = '2011_09_30_drive_0028' ;
msgbox([dataSource,'. 滤波参数设置：新 误差方程']);
    switch dataSource
        case '2011_09_30_drive_0028'
           [ P_ini,Q_const,R_ini,matchN_toplimit,NavFilterParameter ] = GetFilterParameter_SINSerror_dMove_2011_09_30_drive_0028_E( pg,ng,pa,na,NavFilterParameter ) ;
        ...
        case 'visual scence'
            [ P_ini,Q_const,R_ini,NavFilterParameter ] = GetFilterParameter_SINSerror_dMove_visualScene_D( pg,ng,pa,na,NavFilterParameter ) ;
        otherwise
...
function [ P_ini,Q_const,R_ini,matchN_toplimit,NavFilterParameter ] = GetFilterParameter_SINSerror_dMove_2011_09_30_drive_0028_E( pg,ng,pa,na,NavFilterParameter )
%%
szj1 = 1/3600*pi/180 * 10;
szj2 = 1/3600*pi/180 * 10;
szj3 = 1/3600*pi/180 * 10;
vns_beita = [1 1 1]*pi/180*0.1 ;
vns_Tlr = [1 1 1]/1000*1 ;
pg = [ 1 1 1 ]*pi/180/3600 * 0.05 ;        % 
pa = [ 1 1 1 ]*1e-6*9.8 *0.01 ;
P_ini = diag([(szj1)^2,(szj2)^2,(szj3)^2,(1e-3)^2,(1e-3)^2,(1e-3)^2,(1e-4)^2,(1e-4)^2,(1e-4)^2,...
                (pg(1))^2,(pg(2))^2,(pg(3))^2,(pa(1))^2,(pa(2))^2,(pa(3))^2,...
                (vns_beita(1))^2,(vns_beita(2))^2,(vns_beita(3))^2,(vns_Tlr(1))^2,(vns_Tlr(2))^2,(vns_Tlr(3))^2 ]); %  15*15
 NavFilterParameter.P_ini_augment_dRdT =  sprintf('%1.1e ',P_ini) ;
...
R_list = [R_list_input,{'1e0'...
                        '1e-1'}];      % 圆周360m Rbb 20.6"

[Selection,ok] = listdlg('PromptString','特征点相机系坐标-噪声方差的幅值:','SelectionMode','single','ListSize',[350,100],'ListString',R_list);
if ok==0    
    Selection = 1 ;
end
answer = inputdlg({'特征点相机系坐标-噪声方差的幅值                     .','特征点个数上限'},'R_ini',1,[ R_list(Selection),'50' ]);
R_ini = diag(eval(answer{1})) ;   % R_const
NavFilterParameter.R_ini_augment_dRdT = answer{1} ;

matchN_toplimit = eval(answer{2}) ;
NavFilterParameter.matchN_toplimit = matchN_toplimit ;
