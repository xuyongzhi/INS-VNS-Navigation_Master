...
% 设置GUI界面的大小和位置
set(handles.ResultDisplay,'Position',[0 90 190 440]);%设置初始位置和大小
...
if ~isempty(data)
    ...
    xlabel('时间/S','fontsize',handles.titleFontSize);
    title(AddBias([project,'-',comment]),'fontsize',handles.titleFontSize);     %以中文说明为plot标题
    ylabel(name,'fontsize',handles.titleFontSize);    %以变量名为轴名
...
xlabel('x方向/m','fontsize',handles.titleFontSize);
ylabel('y方向/m','fontsize',handles.titleFontSize);
...
if ~isempty(data)
    ...
    if strcmp(name,'position(m)') && ~strcmp(project,'contrast')
        ...
        xlabel('x方向/m','fontsize',handles.titleFontSize);
        ylabel('y方向/m','fontsize',handles.titleFontSize);
        title(AddBias([project,'-轨迹']),'fontsize',handles.titleFontSize);     %以中文说明为plot标题
        saveas(track_h,saveHand([figureOutputPath,'\',project,'-',name,'.fig']));   % 以“方案名-变量名”作为.fig名
        saveas(track_h,saveHand([figureOutputPath,'\',project,'-',name,'.emf']));
    end
...
for j=1:difNameNum
    ...
    combineLength(j)=combineLength(j)-1;    % 不然容错性比较差
end
...
for j=1:difNameNum
    ...
    if isNeedCombine == 1
        ...
        for j_sub=1:j_newCellNum
            ...
            data_j_sub(1,1:combineLength(j)) = toCombineData(1:combineLength(j)) ;  % 数据合并第一步
            %%%%%%%%%%%%%%%%%%%%% ************ %%%%%%%%%%%%%%%
            ...
            for k=firstProjectIndex(j)+1:orNum
                if strcmp(name_j,originalResult{k}.name)
                    ...
                    if ~isempty(data_k)
                        ...
                        data_j_sub(difProjectNum,1:combineLength(j)) = toCombineData(1:combineLength(j)) ;  % 数据合并第k步% 最终确定 data
...
if ~isdir(ResultPath)
   ResultPath = uigetdir(projectDataPath,'选择待绘制结果文件');
end
...
for k=1:dataNum
    ...
    title(AddBias(WholeExplain),'fontsize',handles.titleFontSize);     %以中文说明为plot标题
    legend(ploth,holdTitle,'fontsize',8) ;
    xlabel('时间/S','fontsize',handles.titleFontSize);
    ylabel(WholeName,'fontsize',handles.titleFontSize);    %以变量名为轴名
    v =axis;
    v(3)=v(3)*0.95;
    if v(3)==0
        v(3)=-v(4)/50;
