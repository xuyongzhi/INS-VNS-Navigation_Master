...
function [SINS_Result,errorStr] = main_SINSNav( imuInputData,trueTrace,visualInputData )
format long
...
R_gyroDRbbUKFB = diag([1 1 1 1]*1e-5);
if isTrueX0==1
    gyroDriftEsmB(:,1) = pg ;
    ...
    P_gyroDRbbUKFB(:,:,1) = diag([ (pg(1))^2+1e-5,(pg(2))^2+1e-5,(pg(3))^2+1e-5 ]);
end
...
for t = 1:runTimeNum-1
    ...
    if mod(t-1,cycleT_VNS/imu_T)==0 && t>1
        k_integ = k_integ+1;
        Crb_ins_last = Crb_ins ;
        Crb_ins = Crb ;
        
        R_INS = Crb_ins * Crb_ins_last';
   ...
   %     dbstop in gyroDRbbBUKF
        ...
        opintions.headingScope = 180 ;
        RinsAngle(:,k_integ) = GetAttitude(R_INS,'degree',opintions) ;
        RvnsAngle(:,k_integ) = GetAttitude(R_VNS,'degree',opintions) ;
        trueFCnb = FCbn(true_attitude(:,t+1))' ;
        trueFCnb_last = FCbn(true_attitude(:,t+1-cycleT_VNS/imu_T))' ;
        trueRbb = trueFCnb*trueFCnb_last' ;
        trueRbbAngle(:,k_integ) = GetAttitude(trueRbb,'degree',opintions) ;
        
        RinsAngleError(:,k_integ) = GetAttitude(R_INS*trueRbb','degree',opintions) ;
        RvnsAngleError(:,k_integ) = GetAttitude(R_VNS*trueRbb','degree',opintions) ;
    end
