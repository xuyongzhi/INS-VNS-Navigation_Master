...
validTime = fix(lengthArrayOld(1)/frequencyArray(1));
for k=1:N
    validTime = min(validTime,fix(lengthArrayOld(k)/frequencyArray(k))) ;
end
validLenthArray = zeros(1,N);
for k=1:N
    validLenthArray(k) = min( lengthArrayOld(k),fix(validTime*frequencyArray(k)) );
end
% 组合数据长度
% 组合数据取频率最小的那个数据
