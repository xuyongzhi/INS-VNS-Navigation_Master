...
function [gyroDriftEsmB_new,vnsdRbbEsmB_new,P_gyroDRbbKFB_new] = c_gyroDRbbBUKF(R_INS,R_VNS,T,gyroDriftEsmB_old,vnsdRbbEsmB_old,P_gyroDRbbKFB_old,Q_gyroDRbbKFB,R_gyroDRbbKFB,Wrbb)

Z_gyroDRbbB = FCnbtoQ(R_INS*R_VNS') ;   % 量测量

Fai = eye(6);
[X_new,P_gyroDRbbKFB_new] = UKF([gyroDriftEsmB_old;vnsdRbbEsmB_old],P_gyroDRbbKFB_old,Q_gyroDRbbKFB,R_gyroDRbbKFB,Z_gyroDRbbB,Wrbb,Fai,6,-3,-3,T);
gyroDriftEsmB_new = X_new(1:3) ;
vnsdRbbEsmB_new = X_new(4:6) ;

function Zpre_gyroDRbbB = hfun_DRbbB(X,T,Wrbb)
%% 由状态计算量测
% Wrbb = [0 0 0];
gyroDrift = X(1:3);
RbbVnsConst = X(4:6) ;
...
dRbb_T = FCbn(RbbVnsConst);
DR = (eye(3)-OgyroDrift*T-(OWrbb^2+OgyroDrift*OWrbb)*T^2)*dRbb_T ;   % RINS * RVNS'
