...
for t = 1:runTimeNum-1
    ...
    if mod(t-1,cycleT_VNS/imu_T)==0 && t>1
     ...
     %     dbstop in gyroDRbbBUKF
        [gyroDriftEsmB(:,k_integ+1),vnsdRbbEsmB(:,k_integ+1),P_gyroDRbbUKFB(:,:,k_integ+1)] = c_gyroDRbbBUKF(R_INS,R_VNS,cycleT_VNS,gyroDriftEsmB(:,k_integ),vnsdRbbEsmB(:,k_integ),P_gyroDRbbUKFB(:,:,k_integ),Q_gyroDRbbUKFB,R_gyroDRbbUKFB,Wrbb) ;
...
if  isKnowTrue==1
    ...
    gyroDriftEsmAError = gyroDriftEsmA-repmat(pg,1,length(gyroDriftEsmA)) ;
    gyroDriftEsmBError = gyroDriftEsmB-repmat(pg,1,length(gyroDriftEsmB)) ;
