...
%     position_e(:,t+1) = FZJtoJW(positione0,planet);    % 再转化为经纬高度
    ...
    if mod(t-1,cycleT_VNS/imu_T)==0 && t>1
     ...
     %     dbstop in gyroDRbbBUKF
        [gyroDriftEsmB(:,k_integ+1),P_gyroDRbbUKFB(:,:,k_integ+1)] = gyroDRbbBUKF(R_INS,R_VNS,cycleT_VNS,gyroDriftEsmB(:,k_integ),P_gyroDRbbUKFB(:,:,k_integ),Q_gyroDRbbUKFB,R_gyroDRbbUKFB,Wibb) ;
